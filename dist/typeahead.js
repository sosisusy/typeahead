...
exports.AttributeList = exports.ClassNameList = exports.DEFAULT_CONFIGURE = void 0; // 기본 값
...
var axios_1 = __importDefault(__webpack_require__(18));

var Utils =
/** @class */
function () {
  function Utils() {} // element 찾아서 리턴


  Utils.prototype.findElement = function (target) {
    if (!target) throw new Error("target not found");
    if (_typeof(target) === "object" && target.nodeType == 1) return target;
  ...
  Utils.prototype.findRootElement = function (target) {
    ...
    var find = function find(ele, cnt) {
      if (!ele || cnt >= limit) return null;
      if (ele.className.indexOf(Config_1.ClassNameList.root) !== -1) return ele;
  ...
  Utils.prototype.findStringItem = function (list, value) {
    return lodash_1["default"].filter(list, function (item) {
...
var Typeahead_1 = __importDefault(__webpack_require__(15));

window.Typeahead = Typeahead_1["default"];
exports["default"] = Typeahead_1["default"];
...
var Event = __importStar(__webpack_require__(36));

var primaryIndex = 0; // init

function initialize(defaultConfigure, configure) {
  var mergeConfigure = __assign(__assign({}, defaultConfigure), configure);

  mergeConfigure.target = Utils_1["default"].findElement(mergeConfigure.target); // 대상 없으면 에러

  if (!mergeConfigure.target) throw new Error("target not found"); // 데이터 url로 가져오기

  if (mergeConfigure.lazy) {
    Utils_1["default"].getListData(mergeConfigure.lazy).then(function (res) {
      return mergeConfigure.list = res;
    });
  } // 검색 영역 없으면 에러


  if (!mergeConfigure.list) throw new Error("search area not found"); // 도우미 컨테이너 생성

  makeTypeheadContainer(mergeConfigure); // element 셀렉터 기록
  ...
  mergeConfigure.inputSelector = mergeConfigure.rootSelector + " ." + Config_1.ClassNameList.input; // element 기록
...
function makeTypeheadContainer(configure) {
  var target = configure.target,
      targetParent = target.parentNode,
      targetStyle = window.getComputedStyle(target),
      targetFontSize = Utils_1["default"].getStyleValue(targetStyle, "font-size"),
      targetFontFamily = Utils_1["default"].getStyleValue(targetStyle, "font-family"),
      targetFontWeight = Utils_1["default"].getStyleValue(targetStyle, "font-weight"),
      targetPadding = Utils_1["default"].getStyleValue(targetStyle, "padding"),
      targetBorder = Utils_1["default"].getStyleValue(targetStyle, "border"),
      targetWdith = target.getBoundingClientRect().width;
  var addClassList = {
  ...
  if (addClassList.root) lodash_1["default"].map(addClassList.root, function (customClass) {
    return rootContainer.classList.add(customClass);
  });
  rootContainer.setAttribute(Config_1.AttributeList.privateKey, primaryIndex.toString());
  rootContainer.style.width = targetWdith + "px"; // content

  content.classList.add(Config_1.ClassNameList.content); // list container

  listContainer.classList.add(Config_1.ClassNameList.list);
  if (addClassList.list) lodash_1["default"].map(addClassList.list, function (customClass) {
    return listContainer.classList.add(customClass);
  }); // hint element
...
function default_1(configure) {
  ...
  var rootContainer = CONFIGURE.rootContainer,
      listContainer = CONFIGURE.listContainer,
      hintElement = CONFIGURE.hintElement,
      target = CONFIGURE.inputElement || CONFIGURE.target;
  var targetValue = "",
      hoverItemIndex = -1,
      findList = []; // 텍스트 입력 핸들
  ...
  var handleSearch = function handleSearch(e) {
    ...
    if (CONFIGURE.onSearch) CONFIGURE.onSearch(findList); // 검색 허용 갯수 초과시, 검색된 내용 없을 시
    ...
    Event.showSearchData(findList, CONFIGURE); // 힌트 기록
  ...
  var handleKeyDown = function handleKeyDown(e) {
    ...
    var listChildren = listContainer.children,
        hoverListItem = listContainer.querySelector("." + Config_1.ClassNameList.item + ".hover");
    ...
    var hoverItem = listChildren[hoverItemIndex]; // 검색 리스트 리렌더링

    Event.applyAllListItemStyle(CONFIGURE);
    Event.applyListItemHoverStyle(hoverItem, CONFIGURE); // 힌트 기록
...
var lodash_1 = __importDefault(__webpack_require__(1)); // 리스트 아이템 기본 스타일


var applyListItemStyle = function applyListItemStyle(item, configure) {
...
exports.applyAllListItemStyle = function (configure) {
  var listContainer = configure.listContainer,
      itemList = listContainer.querySelectorAll("." + Config_1.ClassNameList.item);
  lodash_1["default"].map(itemList, function (item) {
    return applyListItemStyle(item, configure);
  });
}; // 리스트 아이템 마우스오버 스타일 적용


exports.applyListItemHoverStyle = function (item, configure) {
  var hintElement = configure.hintElement,
      target = configure.inputElement || configure.target;
  item.classList.add("hover");
  item.style.color = configure.itemHoverColor;
  item.style.backgroundColor = configure.itemHoverBackgroundColor; // 힌트 기록
...
exports.appendListItem = function (type, value, configure) {
  var addClass = configure.addItemClass,
      findKey = configure.key;
  var listContainer = configure.listContainer,
      item = document.createElement("div"); // 클래스 설정

  item.classList.add(Config_1.ClassNameList.item);
  if (addClass) lodash_1["default"].map(addClass, function (customClass) {
    return item.classList.add(customClass);
  }); // 스타일 설정

  applyListItemStyle(item, configure); // 아이템 값 설정
  ...
  item.addEventListener("click", function () {
    return exports.selectListItem(value, configure);
  });
...
exports.showSearchData = function (findList, configure) {
  ...
  listContainer.innerHTML = ""; // 기본 아이템 스타일로 추가

  lodash_1["default"].map(findList, function (item) {
    return exports.appendListItem(_typeof(findList[0]), item, configure);
  });
}; // 검색된 아이템 선택
...
exports.selectListItem = function (item, configure) {
  var target = configure.target,
      targetClone = configure.inputElement,
      findKey = configure.key;
  ...
  switch (_typeof(item)) {
    ...
    case "object":
      ...
      if (targetClone) targetClone.value = item[findKey];
      break;
